...
const RecentBlogs = () => {
  ...
  const [wishlistLoading, setWishlistLoading] = useState({}); // Track loading state for each blog
  ...
  const handleAddToWishlist = async (blog) => {
    setWishlistLoading((prev) => ({ ...prev, [blog._id]: true })); // Set loading for the specific blog
    ...
    try {
      ...
      setWishlistLoading((prev) => ({ ...prev, [blog._id]: false })); // Reset loading for the specific blog
    }
  };
  ...
  return (
    <div className="container py-8 px-4 md:px-8 mx-auto">
      ...
      {loading ? (
        <div className="text-center">Loading blogs...</div> // Loading indicator
      ) : (
          ...
          {blogs.map((blog) => (
            ...
            >
              ...
              <div className="px-4 py-3 flex justify-between items-center border-t">
                ...
                <div>
                  <button
                    className={`px-4 py-2 ${wishlistLoading[blog._id] ? 'bg-gray-300 cursor-not-allowed' : 'bg-gray-300 hover:bg-gray-400'} text-sm font-semibold rounded-md transition duration-300`}
                    disabled={wishlistLoading[blog._id]} // Disable the button for the specific blog
                    onClick={() => handleAddToWishlist(blog)}
                  >
                    {wishlistLoading[blog._id] ? 'Adding...' : 'Add to Wishlist'}
                  </button>
                </div>
              </div>
            </div>
